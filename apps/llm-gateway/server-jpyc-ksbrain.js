/**
 * server-jpyc-ksbrain.js — JPYC x402 payment gateway for Kurage Stock Brain
 *
 * server-jpyc-kcbrain.js(kcbrain用)と同じ検証/決済フロー:
 * JPYC transferWithAuthorization を検証しオンチェーン決済後、
 * ksbrain(:18338)へAPIキーとDeepSeekプロバイダ指定を注入してプロキシする。
 *
 * Ports:
...
const PORT          = Number.parseInt(process.env.PORT_JPYC_KSBRAIN || "18349", 10);
...
if (!KSBRAIN_TOKEN) {
  console.error("KSBRAIN_API_TOKEN (or KSBRAIN_TOKEN) is required");
  process.exit(1);
}

// 公開パス(/ksbrain/...) -> ksbrain upstream パス。7段フル分析はCHAIN価格。
const SKILLS = {
  "/ksbrain/analyze/technical": "/v1/analyze/technical",
  "/ksbrain/analyze/fundamentals": "/v1/analyze/fundamentals",
  "/ksbrain/analyze/disclosure": "/v1/analyze/disclosure",
  "/ksbrain/analyze/news": "/v1/analyze/news",
  "/ksbrain/analyze/market-context": "/v1/analyze/market-context",
  "/ksbrain/debate/bull-bear": "/v1/debate/bull-bear",
  "/ksbrain/assess/risk": "/v1/assess/risk",
};

// 7回のLLM連鎖(technical〜final)+米株ワンショットはEDGAR取得込み。CHAIN価格。
const CHAIN_SKILLS = {
  "/ksbrain/analyze/full": "/v1/analyze/full",
  "/ksbrain/us/analyze/full": "/v1/us/analyze/full",
};
...
console.log(`Stock Brain JPYC gateway — payTo=${JPYC_PAY_TO} amount=${JPYC_AMOUNT} chain=${JPYC_AMOUNT_CHAIN}`);
...
function proxyHealth() {
  return new Promise((resolve) => {
    const url = new URL(`${KSBRAIN_URL}/health`);
...
if (certOk) {
  https.createServer({ cert: fs.readFileSync(TLS_CERT), key: fs.readFileSync(TLS_KEY) }, requestHandler)
    .listen(PORT, HOST, () => console.log(`Stock Brain JPYC gateway → https://${HOST}:${PORT} → ${KSBRAIN_URL}`));
} else {
  http.createServer(requestHandler)
    .listen(PORT, HOST, () => console.log(`Stock Brain JPYC gateway → http://${HOST}:${PORT} → ${KSBRAIN_URL}`));
}
