/**
 * server-jpyc.js — JPYC x402 payment gateway for LLM2API
 *
 * Verifies JPYC transferWithAuthorization, settles on-chain,
 * then proxies the request to the LLM gateway (port 8019).
 *
 * Ports:
 *   This server : PORT_JPYC (default 8020)
 *   Upstream    : PORT_UPSTREAM (default 8019, where server.js runs)
 */
...
const PORT          = Number.parseInt(process.env.PORT_JPYC     || "8020", 10);
...
const UPSTREAM_PORT = Number.parseInt(process.env.PORT_UPSTREAM || "8019", 10);
...
const JPYC_AMOUNT    = process.env.JPYC_AMOUNT || "1500000000000000000"; // 1.5 JPYC
...
console.log(`LLM2API JPYC gateway — payTo=${JPYC_PAY_TO} amount=${JPYC_AMOUNT}`);
...
async function handle(req, res) {
  ...
  const requiresPayment = req.method === "POST" && (
    ...
    url.pathname.startsWith("/llm2api/") ||
...
if (certOk) {
  https.createServer({ cert: fs.readFileSync(TLS_CERT), key: fs.readFileSync(TLS_KEY) }, requestHandler)
    .listen(PORT, HOST, () => console.log(`LLM2API JPYC gateway → https://${HOST}:${PORT} → upstream http://${UPSTREAM_HOST}:${UPSTREAM_PORT}`));
} else {
  http.createServer(requestHandler)
    .listen(PORT, HOST, () => console.log(`LLM2API JPYC gateway → http://${HOST}:${PORT} → upstream http://${UPSTREAM_HOST}:${UPSTREAM_PORT}`));
}
