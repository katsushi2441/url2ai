...
if (isset($_GET['feed'])) {
    ...
    echo '<channel>' . "\n";
    echo '<title>URL2AI ERNIE Image タイムライン | UImageV</title>' . "\n";
    echo '<link>' . $BASE_URL . '/uimagev.php</link>' . "\n";
    echo '<description>X投稿から URL2AI ERNIE Image が ERNIE-Image-Turbo で生成したAI画像のタイムライン。</description>' . "\n";
...
if ($detail_post) {
    $page_title = 'UImage | URL2AI ERNIE Image | ' . $detail_post['tweet_id'];
    $page_description = 'URL2AI ERNIE Image が ERNIE-Image-Turbo で生成した画像。' . mb_substr(str_replace("\n", ' ', isset($detail_post['thread_text']) ? $detail_post['thread_text'] : ''), 0, 120);
    $page_url = $BASE_URL . '/' . $THIS_FILE . '?id=' . urlencode($detail_post['tweet_id']);
} else {
    $page_title = $SITE_NAME . ' — URL2AI ERNIE Image 画像ビュー';
    $page_description = 'X投稿URLをもとに URL2AI ERNIE Image が ERNIE-Image-Turbo で生成した画像を公開表示するビューアです。';
...
<div class="header">
    ...
    <span class="badge">URL2AI</span>Image
...
<div class="container">
    <div class="count-bar">
        <?php echo count($posts); ?> 件の URL2AI ERNIE Image 生成画像
