...
if (isset($_GET['feed'])) {
    ...
    echo '<channel>' . "\n";
    echo '<title>URL2AI ERNIE Image API タイムライン | UImageV</title>' . "\n";
    echo '<link>' . $BASE_URL . '/uimagev.php</link>' . "\n";
    echo '<description>X投稿から URL2AI ERNIE Image API が ERNIE-Image-Turbo で生成したAI画像のタイムライン。</description>' . "\n";
...
if ($detail_post) {
    $page_title = 'UImage | URL2AI ERNIE Image API | ' . $detail_post['tweet_id'];
    $page_description = 'URL2AI ERNIE Image API が ERNIE-Image-Turbo で生成した画像。' . mb_substr(str_replace("\n", ' ', isset($detail_post['thread_text']) ? $detail_post['thread_text'] : ''), 0, 120);
    $page_url = $BASE_URL . '/' . $THIS_FILE . '?id=' . urlencode($detail_post['tweet_id']);
} else {
    $page_title = $SITE_NAME . ' — URL2AI ERNIE Image API 画像ビュー';
    $page_description = 'X投稿URLをもとに URL2AI ERNIE Image API が ERNIE-Image-Turbo で生成した画像を公開表示するビューアです。';
...
<div class="header">
    ...
    <span class="badge">URL2AI ERNIE Image API</span>
...
<div class="container">
    ...
    <div class="detail-body">
        <div class="detail-section-title">🎨 Generated Image by URL2AI ERNIE Image API</div>
...
<div class="container">
    <div class="count-bar">
        <?php echo count($posts); ?> 件の URL2AI ERNIE Image API 生成画像
