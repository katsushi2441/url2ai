...
.avatar{width:40px;height:40px;background:linear-gradient(135deg,#7c3aed,#db2777);border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;color:#fff;flex-shrink:0;}
...
/* 短編小説プレビュー */
.story-block{background:#f5f3ff;border-left:3px solid #7c3aed;border-radius:0 8px 8px 0;padding:12px 14px;margin-bottom:12px;font-size:14px;line-height:1.9;color:#333;white-space:pre-wrap;max-height:120px;overflow:hidden;position:relative;cursor:pointer;}
.story-block::after{content:'';position:absolute;bottom:0;left:0;right:0;height:32px;background:linear-gradient(transparent,#f5f3ff);pointer-events:none;}
.story-block.expanded{max-height:none;}
.story-block.expanded::after{display:none;}
.expand-btn{background:none;border:none;color:#7c3aed;font-size:.75rem;cursor:pointer;padding:2px 0 6px;display:block;}
...
.detail-story{background:#f5f3ff;border-left:3px solid #7c3aed;border-radius:0 8px 8px 0;padding:16px 18px;font-size:15px;line-height:2;color:#222;white-space:pre-wrap;margin-bottom:8px;}
...
<div class="header">
    ...
    <span class="badge">URL2AI Stories</span>
    ...
    <div class="userbar">
        ...
        <a href="?uv_logout=1" class="btn-sm">logout</a>
        <?php else: ?>
        <a href="?uv_login=1" class="btn-sm">X でログイン</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</div>
...
<script>
var uvPosts = <?php echo json_encode(array_values($posts), JSON_UNESCAPED_UNICODE); ?>;
var PAGE_SIZE = 30;
var currentPage = 0;
...
function renderUvPosts(from, to) {
    var list = document.getElementById('post-list');
    if (!list) return;
    for (var i = from; i < to && i < uvPosts.length; i++) {
        var p       = uvPosts[i];
        var tid     = p.tweet_id    || '';
        ...
        var xlink = turl ? '<a href="' + uvEsc(turl) + '" target="_blank" rel="noopener" class="x-link">' + xsvg + '元の投稿</a>' : '';
...
function loadMoreUv() {
    ...
    if (from >= uvPosts.length) {
        document.getElementById('load-indicator').style.display = 'none';
        return;
    }
    renderUvPosts(from, from + PAGE_SIZE);
}
...
if (sentinel) {
    var observer = new IntersectionObserver(function(entries) {
        if (entries[0].isIntersecting) {
            ...
            setTimeout(function() {
                loadMoreUv();
                if (currentPage * PAGE_SIZE >= uvPosts.length) {
                    document.getElementById('load-indicator').style.display = 'none';
                }
            }, 200);
        }
