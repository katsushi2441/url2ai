...
- 実行管理: RQDB4AI `ernie-192-168-0-11-web` / `ernie-192-168-0-11-worker`

`POST /image/generate`はゲートウェイから直接推論サーバーへ送らない。中央の
RQDB4AIへ投入し、Web優先・バックグラウンド次点の2キューを監視する単一workerが
`192.168.0.11:18300/generate`を1件ずつ実行する。RQDB4AI障害時の直接
フォールバックは行わず、503として呼び出し側へ返す。
